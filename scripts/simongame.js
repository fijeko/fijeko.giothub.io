var simongame = {
    ...
    on_window_load : function(){
        document.getElementById('b_start')
            .addEventListener( 'click', simongame.start, false );
        document.getElementById('b_stop')
            .addEventListener( 'click', simongame.stop, false );
        simongame.stop_mode();
    },
    ...
    play_sound : function(sound) {
        ...
        return function(){
            if( simongame.mode != 'play' )  return;
            document.getElementById( 'b_snd' + s )
                .classList.add('clr-active-f'+s);
            document.getElementById( 'snd_'  + s ).play();
            setTimeout( function(){
                document.getElementById( 'b_snd' + s )
                    .classList.remove('clr-active-f'+s);
            }, 200);
        }
    },
...
console.log('stop_listen' );
    ...
    output : function(){
        ...
        document.getElementById('p_step').innerHTML =
            simongame.pattern.length;
    }
}
